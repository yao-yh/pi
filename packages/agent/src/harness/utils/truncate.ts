 ...
 * 工具输出共用的截断工具。
 *
 * 截断由两个独立限制决定，以先达到的限制为准：
 * - 行数限制（默认 2000 行）
 * - 字节数限制（默认 50KB）
 *
 * 除 Bash 尾部截断的边界情况外，不会返回不完整的行。
 */
...
/**
 * 从内容尾部截断，保留开头 N 行或指定字节数。
 * 适用于需要查看文件开头的读取场景。
 *
 * 不会返回不完整的行。如果第一行超过字节限制，
 * 则返回空内容，并将 firstLineExceedsLimit 设为 true。
 */
export function truncateHead(content: string, options: TruncationOptions = {}): TruncationResult {
	...
	for (let i = 0; i < lines.length && i < maxLines; i++) {
		const line = lines[i];
		const lineBytes = utf8ByteLength(line) + (i > 0 ? 1 : 0); // 为换行符增加 1 字节
...
/**
 * 从内容头部截断，保留末尾 N 行或指定字节数。
 * 适用于需要查看末尾内容（错误、最终结果）的 Bash 输出。
 *
...
export function truncateTail(content: string, options: TruncationOptions = {}): TruncationResult {
	...
	for (let i = lines.length - 1; i >= 0 && outputLinesArr.length < maxLines; i--) {
		const line = lines[i];
		const lineBytes = utf8ByteLength(line) + (outputLinesArr.length > 0 ? 1 : 0); // 为换行符增加 1 字节
