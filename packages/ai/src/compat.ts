/**
 * 临时兼容入口，用于保留旧版全局 pi-ai API：包括注入环境变量 API 密钥的
 * api-dispatch `stream()`/`complete()`、api-registry、生成目录读取
 * （`getModel`/`getModels`/`getProviders`）、各 API 的延迟流包装器和图像生成。
 *
 * 现有应用只需将导入从 "@earendil-works/pi-ai" 切换为
 * "@earendil-works/pi-ai/compat"，其他内容保持不变；新代码使用 `createModels()`
 * 和提供商工厂。完成 coding-agent ModelManager 迁移时删除此模块。
 */
...
/**
 * 将内置 API 实现注册到 api-registry 中，但不覆盖现有条目：加载 compat 前，
 * 测试或扩展可能已经为某个内置 API id 注册了替代实现。
 */
