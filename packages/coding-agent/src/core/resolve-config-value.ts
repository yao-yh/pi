...
/**
 * 将配置值（API 密钥、请求头值等）解析为实际值。
 * - 以 "!" 开头时，将其余内容作为 shell 命令执行并使用 stdout（带缓存）
 * - 使用具名环境变量插值 "$ENV_VAR" 或 "${ENV_VAR}" 引用
 * - 非命令值中，"$$" 转义字面量 "$"，"$!" 转义字面量 "!"
 * - 其他情况按字面值处理
 */
